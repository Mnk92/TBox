...
namespace Mnk.TBox.Plugins.Market.Client.Components.Uploaders
{
    ...
    public partial class PluginUploaderDialog
    {
        ...
        private readonly ObservableCollection<FilePathData> currentPlugins = new();

        private readonly ObservableCollection<Data> currentDependencies = new();

        private readonly PathTemplates pluginsTemplates = new(
        ...
        private readonly Templates dependenciesTemplates = new(
        ...
        public PluginUploaderDialog()
        {
            ...
            lbPlugins.ConfigureSelector(
                ...
                new InputFilePath(
                    "Plugins",
                    pluginsTemplates,
                    text => currentPlugins.IsUniqueIgnoreCase(s => s.ToString(), Path.GetFileName(text)),
                    () => null
                    )
                );
            currentPlugins.CollectionChanged += PluginsChanged;
            ...
            lbDependencies.ConfigureSelector<Data>(
                ...
                new DependenciesSelector("Dependencies",
                    ...
                    x => currentDependencies.IsUniqueIgnoreCase(a => a.ToString(), x),
                    () => null
                    )
                );
            currentDependencies.CollectionChanged += DependenciesChanged;
        }
        ...
        private void RefreshData(object sender, RoutedEventArgs e)
        {
            ...
            btnUpgrade.IsEnabled = btnDelete.IsEnabled && (IsPluginNameValid() || currentPlugins.Count == 0);
        ...
        protected virtual void OnNameChangedToExist(Plugin item)
        {
            ...
            lDate.Content = item.Date.ToDateTime().ToShortDateString();
        ...
        protected virtual void OnReloadData(MarketService.MarketServiceClient client)
        {
            Mt.Do(this, () =>
            {
                cmbAuthor.Value = Environment.UserDomainName + "\\" + Environment.UserName;
                baseUploader.GetList(client, cmbAuthor.Value);
        ...
        protected bool IsPluginNameValid()
        {
            ...
            var name = (cmbName.Value + Constants.PluginExtension).ToUpper();
            return currentPlugins.Count(x => x.ToString().ToUpper() == name) == 1;
        }
        ...
        private void btnUpload_Click(object sender, RoutedEventArgs e)
        {
            ActionWithButton(btnUpload, "upload",
                () => baseUploader.Upload(cmbName.Value, currentPlugins.Select(x => x.Key).ToArray())
            );
        }
        ...
        private void btnUpgrade_Click(object sender, RoutedEventArgs e)
        {
            ActionWithButton(btnUpgrade, "upgrade",
                () => baseUploader.Upgrade(cmbName.Value, currentPlugins.Select(x => x.Key).ToArray())
            );
        }
