...
namespace Mnk.TBox.Core.Contracts
{
    ...
    public sealed class PluginInfoAttribute : Attribute
    {
        ...
        private static ResourceManager GetResourceManager(Type type)
        {
            ...
            if (p == null) throw new ArgumentException("Invalid resource manager " + type);
            return (ResourceManager)p.GetValue(p.DeclaringType, null);
        }

        public string Name => textsResMan.GetString("PluginName");

        public string Description => textsResMan.GetString("PluginDescription");

        public Icon Icon => (Icon)iconsResMan.GetObject("Icon");

        public bool IsIconSystem => SystemIconId > -1;
